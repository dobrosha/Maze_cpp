...
void MazeArea::drawFromFile(QString filename) {
  clearMaze();
  clearPath();
  facade_->maze()->set_from_file(filename.toStdString());
  drawMaze();
}

void MazeArea::saveMaze(QString filename) {
  facade_->maze()->save_maze(filename.toStdString());
}

void MazeArea::drawGenerate(int width, int height) {
  clearMaze();
  clearPath();
  facade_->maze()->set_size(width, height);
  facade_->maze()->generate_maze();
  drawMaze();
}
...
void MazeArea::drawMaze() {
  ...
  // Draw walls
  cellWidth_ = 500.0 / facade_->maze()->get_cols();
  cellHeight_ = 500.0 / facade_->maze()->get_rows();
  for (int i = 0; i < facade_->maze()->get_rows(); i++) {
    for (int j = 0; j < facade_->maze()->get_cols(); j++) {
      int x0 = cellWidth_ * j;
      int y0 = cellHeight_ * i;
      int x1 = cellWidth_ * (j + 1);
      int y1 = cellHeight_ * (i + 1);
      if (facade_->maze()->get_vertical_walls()(i, j)) {
        if (j + 1 == facade_->maze()->get_cols()) {
          x0--;
          x1--;
        }
        painter.drawLine(x1, y0, x1, y1);
      }
      if (facade_->maze()->get_under_walls()(i, j)) {
        if (i + 1 == facade_->maze()->get_rows()) {
          y0--;
          y1--;
        }
        if (j + 1 == facade_->maze()->get_cols()) {
