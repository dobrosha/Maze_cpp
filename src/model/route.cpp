...
void Pacman::getMaze(Maze *maze) {
  uWall_ = &(maze->get_under_walls());
  vWall_ = &(maze->get_vertical_walls());
  rows_ = maze->get_rows();
  cols_ = maze->get_cols();
  allocateMap();
}
...
std::vector<Position> Pacman::findWay(Position begin, Position end) {
  ...
  if (map_[y][x] != -1) {
    ...
    while (y != begin.y || x != begin.x) {
      if (y < rows_ - 1 && !(*uWall_)(y, x) && map_[y + 1][x] == step - 1)
        y++;
      else if (y > 0 && !(*uWall_)(y - 1, x) && map_[y - 1][x] == step - 1)
        y--;
